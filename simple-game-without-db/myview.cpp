...
void MyView::initView()
{
    ...
    //当使用QPalette为窗口设定背景并使用QPalette::Window时必须调用setAutoFillBackground(true)
    ...
    QLineEdit *lineEdit=new QLineEdit(option);
    lineEdit->move(150,250);
    lineEdit->show();
    ...
    gameOverText->setHtml(tr("<font color=white>游戏结束，是否保存？</font>"));
    gameOverText->setFont(QFont("Times", 30, QFont::Bold));
    gameOverText->setPos(220, 100);
    gameOverText->setZValue(2);
    gameOverText->hide();

    //保存游戏分数文本
    saveScoresText =new QGraphicsTextItem(0, scene);
    saveScoresText->setHtml(tr("<font color=white>请输入用户名：</font>"));
    saveScoresText->setFont(QFont("Times", 30, QFont::Bold));
    saveScoresText->setPos(220, 100);
    saveScoresText->setZValue(2);
    saveScoresText->hide();
    // 游戏中使用的按钮

    QPushButton *button1 = new QPushButton(tr("开    始"));
    QPushButton *button2 = new QPushButton(tr("选    项"));
    QPushButton *button3= new  QPushButton(tr("登    录"));
    ...
    QPushButton *button10= new QPushButton(tr("是"));
    QPushButton *button11= new QPushButton(tr("否"));
    QPushButton *button12= new QPushButton(tr("确认"));
    QPushButton *button13= new QPushButton(tr("取消"));


    connect(button1, SIGNAL(clicked()), this, SLOT(startGame()));
    connect(button2, SIGNAL(clicked()), option, SLOT(show()));
    connect(button3,SIGNAL(clicked()),this,SLOT(loginSlots()));
    ...
    connect(button13,SIGNAL(clicked()), this, SLOT(finishGame()));
    connect(button8, SIGNAL(clicked()), this, SLOT(returnGame()));
    connect(button9, SIGNAL(clicked()), this, SLOT(finishGame()));
    connect(button10,SIGNAL(clicked()), this, SLOT(inputName()));
    connect(button11,SIGNAL(clicked()), this, SLOT(finishGame()));
    connect(button12,SIGNAL(clicked()), this, SLOT(saveScores()));

    startButton = scene->addWidget(button1);
    optionButton = scene->addWidget(button2);
    loginButton=scene->addWidget(button3);
    ...
    yesButton=scene->addWidget(button10);
    noButton=scene->addWidget(button11);
    saveScoresButton=scene->addWidget(button12);
    cancelButton=scene->addWidget(button13);

    startButton->setPos(370, 200);
    optionButton->setPos(370, 250);
    loginButton->setPos(370,300);
    ...
    yesButton->setPos(370, 200);
    noButton->setPos(370,250);
    saveScoresButton->setPos(370,200);
    cancelButton->setPos(370,250);

    startButton->setZValue(2);
    optionButton->setZValue(2);
    loginButton->setZValue(2);
    exitButton->setZValue(2);
    restartButton->setZValue(2);
    returnButton->setZValue(2);
    finishButton->setZValue(2);
    yesButton->setZValue(2);
    noButton->setZValue(2);
    saveScoresButton->setZValue(2);
    cancelButton->setZValue(2);
...
/*连接数据库*/
QSqlDatabase MyView::OpenDatabase()
{
...
void MyView::saveScores()
{
    QSqlDatabase db=OpenDatabase();
    QSqlQuery query(db);
    query.prepare("insert into score(score) values (:totalScore)");
    query.bindValue(":totalScore",gameScoreText->toPlainText().toInt());
    if(!query.exec())
